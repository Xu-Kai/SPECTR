...
__ONMIC__ void mic_extend_a_kmer(const char* kmer, const char* sequence, uint32_t index_kmer, const uint32_t& index_last_mod, 
    ...
    while(index_pos > 0 || flag == 1){
        ...
        if ((index_kmer == (org_boundary_left - 1)) || (index_kmer == (org_boundary_right - 1)) || 
            ...
            if(one_path_query_text(kmer_new, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width) == true){ 
...
__ONMIC__ void mic_correct_errors_between_solid_regions(const char* org_sequence, char* sequence_modified, const char* quality_score, const uint32_t left_first, 
        ...
        if(pos_path_vec[it_path * READ_MAX_LEN] == 0){
            ...
            if(index_last_modified_base > index_last_mod){
                ...
                for(uint32_t it_check = index_last_mod; it_check<= index_last_modified_base; it_check ++){
                    ...
                    if(one_path_query_text(kmer_current, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width) == true){
                        num_success += 1;
                    }else{
                        num_fails += 1;
...
__ONMIC__ void mic_extend_a_kmer_5_prime_end(const char* kmer, const char* sequence, uint32_t index_kmer, uint32_t* pos_path_vec,
...
__ONMIC__ void mic_correct_errors_5_prime_end(const char* org_sequence, char* sequence, const char* quality_score, const uint32_t& index_start, char* sequence_modification, uint32_t& trim_5_end,
...
__ONMIC__ inline void mic_extend_a_kmer_3_prime_end(const char* kmer, const char* sequence, uint32_t index_kmer, uint32_t* pos_path_vec, 
        char* c_path_vec, uint32_t& c_path_size, const uint32_t& org_boundary, const char* quality_score, const uint8_t* bit_vector, 
...
__ONMIC__ void mic_correct_errors_first_kmer(const char* sequence, const char* quality_score, char* sequence_modification, uint32_t* pos_path_vec, char* c_path_vec, 
        ...
        if(low_qs_indexes_ind == 0){
            ...
            if(one_path_query_text(first_kmer, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width) == true){
                //save a path
...
__ONMIC__ void mic_solid_first_kmer(uint32_t* pos_path_vec, char* c_path_vec, const char* sequence, bool& extension_success, uint8_t* bit_vector, uint32_t* hash_seed, 
...
__ONMIC__ void mic_extend_first_kmer_to_right(const char* sequence, const char* quality_score, uint32_t* pos_path_vec_first, char* c_path_vec_first,
   ...
   // memcpy(second_kmer, first_kmer + 1, arg.kmer_length);
    ...
    if(one_path_query_text(second_kmer, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width) == true){
        ...
        bool pass_result[4];
        uint32_t ii = 0;
        path_query_text(second_kmer, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width, sequence[arg.kmer_length], 0, pass_result);
...
__ONMIC__ void mic_correct_errors(char* read_content, char* read_quality, char* sequence_modification,
	...
	//for(int i = 0; i < solid_regions_ind; i ++){
	//	printf("%d %d \n", solid_regions1[i], solid_regions2[i]);
	//}
	//printf("*****************\n");
