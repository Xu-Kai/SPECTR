...
__ONMIC__ void mic_extend_a_kmer(const char* kmer, const char* sequence, uint32_t index_kmer, const uint32_t& index_last_mod, 
  ...
  while(index_pos > 0 || flag == 1){
    ...
    if ((index_kmer == (org_boundary_left - 1)) || (index_kmer == (org_boundary_right - 1)) || 
      ...
      path_query_text(kmer_new, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width, 1 , 0, pass_result);
      for(unsigned short int it_alter = A; it_alter <= T; it_alter ++){ 
        kmer_new[arg.kmer_length - 1] = NEOCLEOTIDE[it_alter];
        //bool tmp = query_text(kmer_new, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
        //if(pass_result[it_alter] != tmp)
        //   printf("path serch wrong\n");
        if(pass_result[it_alter] == true){
      ...
      bool tmp =  one_path_query_text(kmer_new, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
      //bool tmp1 = query_text(kmer_new, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width); 
      //if(tmp != tmp1)
      //   printf("one path wrong\n");
      if(tmp == true){ 
        path_content[(index_pos << 2) + k_ind ++] = kmer_new[arg.kmer_length - 1];      
      }else{
        path_query_text(kmer_new, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width, 1 , sequence[index_kmer + arg.kmer_length], pass_result);
        int ii = 0;
        for(unsigned short int it_alter = A; it_alter <= T; it_alter ++){ 
          kmer_new[arg.kmer_length - 1] = NEOCLEOTIDE[it_alter];
          if(sequence[index_kmer + arg.kmer_length] != NEOCLEOTIDE[it_alter]){
            //bool tmp = query_text(kmer_new, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
            // if(tmp != pass_result[ii++])
            //    printf("wrong\n");
            if(pass_result[ii ++] == true){
              path_content[(index_pos << 2) + k_ind ++] = NEOCLEOTIDE[it_alter];      
            }
          }
        }      
...
__ONMIC__ void mic_modify_sequence(const char* org_sequence, char* sequence_modified, const char* quality_score, char* sequence_modification, 
...
__ONMIC__ void mic_correct_errors_between_solid_regions(const char* org_sequence, char* sequence_modified, const char* quality_score, const uint32_t left_first, const uint32_t index_start, const uint32_t index_end, const uint32_t right_second, const uint32_t org_boundary_left, const uint32_t org_boundary_right, char* sequence_modification, uint32_t& trim_5_end, uint32_t& trim_3_end, uint32_t& num_solid_islands, const uint32_t read_length, uint32_t max_trimmed_bases, uint32_t min_check_length, uint32_t& num_corrected_errors_local, const uint8_t* bit_vector, const uint32_t* hash_seed, init_args& arg, func_mem& funcm){
  ...
  bool pass_result[4];
  path_query_text(kmer_initial, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width, 1 , 0, pass_result);
  for(uint16_t it_alter = A; it_alter <= T; it_alter ++){
    kmer_initial[arg.kmer_length - 1] = NEOCLEOTIDE[it_alter];
    //new path 
    if(run_exploration == true){ 
      //bool tmp  = query_text(kmer_initial, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
      //if(pass_result[it_alter] != tmp)
    ...
    if(pos_path_vec[it_path * READ_MAX_LEN] == 0){
      ...
      if(index_last_modified_base > index_last_mod){
        ...
        for(uint32_t it_check = index_last_mod; it_check<= index_last_modified_base; it_check ++){
          ...
          bool tmp = one_path_query_text(kmer_current, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width); 
          //bool tmp1 = query_text(kmer_current, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
          //if(tmp != tmp1)
          //    printf("one path wrong\n");
          if(tmp == true){
            num_success += 1;
  ...
  if(all_solid_wo_modification){
    ...
    /*
       ...
       */    
    mic_modify_sequence(org_sequence, sequence_modified, quality_score, sequence_modification, trim_5_end, trim_3_end, 
...
__ONMIC__ void mic_extend_a_kmer_5_prime_end(const char* kmer, const char* sequence, uint32_t index_kmer, uint32_t* pos_path_vec, char* c_path_vec, uint32_t& c_path_size,  const uint32_t& org_boundary, const char* quality_score, const uint8_t* bit_vector, const uint32_t* hash_seed, bool& run_exploration, init_args& arg, func_mem& funcm){
  ...
  while(index_pos > 0 || flag == 1){
    ...
    if ((index_kmer == (org_boundary + 1)) || ((quality_score[index_kmer - 1] - arg.quality_score_offset) <= arg.extremely_low_quality_score)){
      path_query_text(kmer_new, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width, 0 , 0, pass_result);
      for(unsigned short int it_alter = A; it_alter <= T; it_alter ++){ 
        kmer_new[0] = NEOCLEOTIDE[it_alter];
        //bool tmp = query_text(kmer_new, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
        //if(pass_result[it_alter] != tmp)
        //    printf("path query wrong\n");
        if(pass_result[it_alter] == true){
          path_content[(index_pos << 2) + k_ind ++] = NEOCLEOTIDE[it_alter];      
        }
      }                
      ...
      bool tmp = one_path_query_text(kmer_new, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
      //bool tmp1 = query_text(kmer_new, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
      //if(tmp != tmp1)
      //    printf("one path wrong\n");
      if(tmp == true){ 
        path_content[(index_pos << 2) + k_ind ++] = kmer_new[0];      
      }else{
        path_query_text(kmer_new, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width, 0 , sequence[index_kmer - 1], pass_result);
        int ii = 0;
        for(unsigned short int it_alter = A; it_alter <= T; it_alter ++){ 
          kmer_new[0] = NEOCLEOTIDE[it_alter];
...
__ONMIC__ void mic_correct_errors_5_prime_end(const char* org_sequence, char* sequence, const char* quality_score, const uint32_t& index_start, char* sequence_modification, uint32_t& trim_5_end, uint32_t& trim_3_end, const uint32_t& org_boundary, const uint32_t& read_length, const uint32_t& max_trimmed_bases, const uint32_t& min_check_length, uint32_t& num_corrected_errors_local, const uint8_t* bit_vector, const uint32_t* hash_seed, init_args& arg, func_mem& funcm){
  //vector_candidate_path candidate_path_vector_tmp;
  //init_vector_candidate_path(candidate_path_vector_tmp);
  ...
  //printf("%d \n", index_start);
  bool pass_result[4];
  path_query_text(kmer_initial, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width, 0 , 0, pass_result);
  for(unsigned short int it_alter = A; it_alter <= T; it_alter ++){
  ...
  if(all_solid_wo_modification == true){
    ...
    mic_modify_sequence(org_sequence, sequence, quality_score, sequence_modification, trim_5_end, trim_3_end, 
...
inline __ONMIC__ void mic_extend_a_kmer_3_prime_end(const char* kmer, const char* sequence, uint32_t index_kmer, uint32_t* pos_path_vec, char* c_path_vec, uint32_t& c_path_size, const uint32_t& org_boundary, const char* quality_score, const uint8_t* bit_vector, uint32_t* hash_seed, bool& run_exploration, uint32_t read_length, init_args& arg, func_mem& funcm){
      ...
      for(uint32_t it = 0; it < k_ind; it ++){
        ...
        */
        ...
        /*             
                       ...
                       intf("%c", c_path_vec[c_path_size * READ_MAX_LEN + ii  + 1]);
                       }
                       printf("\n******\n");
                       */
        c_path_size += 1;        
...
__ONMIC__ void mic_correct_errors_3_prime_end(char* org_sequence, char* sequence, char* quality_score, const uint32_t index_start, char* sequence_modification, uint32_t& trim_5_end, uint32_t& trim_3_end, const uint32_t org_boundary, const uint32_t read_length, const uint32_t max_trimmed_bases, const uint32_t min_check_length, uint32_t& num_corrected_errors_local, uint8_t* bit_vector,
...
__ONMIC__ void mic_correct_errors_first_kmer(const char* sequence, const char* quality_score, char* sequence_modification, uint32_t* pos_path_vec, char* c_path_vec, uint32_t& c_path_size, uint8_t* bit_vector, uint32_t* hash_seed, init_args& arg, func_mem& funcm){
  //char first_kmer[36];
  ...
  if(low_qs_indexes_ind == 0){
    ...
    bool tmp = one_path_query_text(first_kmer, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
    //bool tmp1 = query_text(first_kmer, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
    //if(tmp1 != tmp)
    //    printf("path wrong\n");
    if(tmp == true){
      //save a path
      ...
      for(uint32_t it_alter1 = A; it_alter1 <= T; it_alter1 ++){
        ...
        for(uint32_t it_alter2 = A; it_alter2 <= T; it_alter2 ++){
          ...
          //    printf("%c", first_kmer[ii]);
          //printf("\n");

          pos_path_query_text(first_kmer, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func,  arg.bit_vector_width, low_qs_indexes[2], pass_result);

          for(uint32_t it_alter3 = A; it_alter3 <= T; it_alter3 ++ ){
            first_kmer[low_qs_indexes[2]] = NEOCLEOTIDE[it_alter3];
            //bool tmp = query_text(first_kmer, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width); 
            //if(pass_result[it_alter3] != tmp)
            //    printf("pos path search wrong %x %x\n", pass_result[it_alter3], tmp);
            if(pass_result[it_alter3] == true){
      ...
      for(uint32_t it_bases = 0; it_bases < arg.kmer_length; it_bases ++){
        ...
        //printf("it bases = %d\n", it_bases);
        pos_path_query_text(first_kmer, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func,  arg.bit_vector_width, it_bases, pass_result);
        //printf("pass result = %x %x %x %x\n", pass_result[0], pass_result[1], pass_result[2], pass_result[3]);
        for(uint32_t it_alter = A; it_alter <= T; it_alter ++){
    ...
    if(low_qs_indexes_ind <= MAX_LOW_QS_BASES){
      switch(low_qs_indexes_ind){
          ...
          for(uint32_t it_alter1 = A; it_alter1 <= T; it_alter1 ++){
            ...
            for(uint32_t it_alter2 = A; it_alter2 <= T; it_alter2 ++){
              ...
              for(uint32_t it_alter3 = A; it_alter3 <= T; it_alter3 ++) {
                first_kmer[low_qs_indexes[2]] = NEOCLEOTIDE[it_alter3];
                //bool tmp = query_text(first_kmer, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
                //if(tmp != pass_result[it_alter3])
                //    printf("a pos path query wrong\n");
                //printf("it alter %d\n", it_alter3);
                if(pass_result[it_alter3] == true){
                  //save path                                
                  pos_path_vec[c_path_size * 4] = 0;
        ...
        case 2:
          for(uint32_t it_alter1 = A; it_alter1 <= T; it_alter1 ++){
            first_kmer[low_qs_indexes[0]] = NEOCLEOTIDE[it_alter1];
            pos_path_query_text(first_kmer, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func,  arg.bit_vector_width, low_qs_indexes[1], pass_result);
            for(uint32_t it_alter2 = A; it_alter2 <= T; it_alter2 ++){
              first_kmer[low_qs_indexes[1]] = NEOCLEOTIDE[it_alter2];
              //bool tmp = query_text(first_kmer, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
              //if(tmp != pass_result[it_alter2])
              //    printf("a pos path query wrong\n");
              //printf("it alter %d\n", it_alter3);
              if(pass_result[it_alter2] == true){
                //save path                                
                pos_path_vec[c_path_size * 4] = 0;
                if(sequence[low_qs_indexes[0]] != NEOCLEOTIDE[it_alter1]){
                  pos_path_vec[c_path_size * 4 + pos_path_vec[c_path_size * 4] + 1] = low_qs_indexes[0]; 
          ...
          break;
        case 1:
          pos_path_query_text(first_kmer, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func,  arg.bit_vector_width, low_qs_indexes[0], pass_result);
          for(uint32_t it_alter1 = A; it_alter1 <= T; it_alter1 ++){
            first_kmer[low_qs_indexes[0]] = NEOCLEOTIDE[it_alter1];

            //bool tmp = query_text(first_kmer, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
            //if(tmp != pass_result[it_alter1])
            //        printf("a pos path query wrong\n");
            //printf("it alter %d\n", it_alter3);
            if(pass_result[it_alter1] == true){
              //save path
              pos_path_vec[c_path_size * 4] = 0;
              if(sequence[low_qs_indexes[0]] != NEOCLEOTIDE[it_alter1]){
                pos_path_vec[c_path_size * 4 + pos_path_vec[c_path_size * 4] + 1] = low_qs_indexes[0]; 
      ...
      if(c_path_size == 0){
        //bool tmp = query_text(first_kmer, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
        bool tmp = one_path_query_text(first_kmer, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width); 
        //if(tmp != tmp1)
        //    printf("one path query wrong\n");
        if(tmp == true){
      ...
      bool tmp = one_path_query_text(first_kmer, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
      //bool tmp = query_text(first_kmer, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
      //if(tmp1 != tmp)
      //    printf("on path query wrong\n");
      if(tmp == true){
        //save nothing path
        pos_path_vec[c_path_size * 4] = 0;
        c_path_size += 1;;
...
__ONMIC__ void mic_solid_first_kmer(uint32_t* pos_path_vec, char* c_path_vec, const char* sequence, bool& extension_success, uint8_t* bit_vector, uint32_t* hash_seed, init_args& arg, func_mem& funcm){
...
inline __ONMIC__ void mic_extend_a_kmer_3_prime_end(const char* kmer, const char* sequence, uint32_t index_kmer, uint32_t* pos_path_vec, char* c_path_vec, uint32_t& c_path_size, uint32_t* pos_path_vec_first, char* c_path_vec_first,const uint32_t& org_boundary, const char* quality_score, const uint8_t* bit_vector, uint32_t* hash_seed, bool& run_exploration, uint32_t read_length, init_args& arg, func_mem& funcm){
...
__ONMIC__ void mic_extend_first_kmer_to_right(const char* sequence, const char* quality_score, uint32_t* pos_path_vec_first, char* c_path_vec_first,
  ...
  bool tmp = one_path_query_text(second_kmer, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);

  //bool tmp = query_text(second_kmer, bit_vector, &hash_seed[0], arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
  //if(tmp != tmp1)
  //    printf("one path query wrong\n");
  if(tmp == true){
    ...
    uint32_t ii = 0;
    path_query_text(second_kmer, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width, 1, sequence[arg.kmer_length], pass_result);
    for(unsigned short int it_alter = A; it_alter <= T; it_alter ++){
      if(sequence[arg.kmer_length] != NEOCLEOTIDE[it_alter]){
        ...
        /*
           ...
           */
        if(run_exploration == true){
          // bool tmp = query_text(second_kmer, bit_vector, hash_seed, arg.kmer_length, arg.num_hash_func, arg.bit_vector_width);
          // if(tmp != pass_result[ii ++])
          //     printf("path query wrong\n");
          if(pass_result[ii ++] == true){
            //generate new path
...
__ONMIC__ void mic_correct_errors(char* read_content, char* read_quality, char* sequence_modification, uint32_t* hash_seed, uint8_t* bit_vector, init_args& arg, uint32_t& num_corrected_errors_local,uint32_t& trim_5_end, uint32_t& trim_3_end, uint32_t max_trimmed_bases,uint32_t min_check_length, uint32_t read_length, int ssss, func_mem& funcm) {
    ...
    if(solid_regions_ind > 0 && short_non_solid_region == false){
        ...
        for(uint32_t it_region = 1; it_region < solid_regions_ind; it_region ++){
          if((solid_regions1[it_region] - 1) - (solid_regions2[it_region - 1] + 1) + 1 >= arg.kmer_length){
            if(solid_regions2[it_region - 1] + arg.kmer_length < read_length - trim_3_end && solid_regions1[it_region] > trim_5_end){
              ...
              mic_correct_errors_between_solid_regions(
                  read_content,
                  sequence_modified,
                  read_quality,
      ...
      if(solid_regions_ind >= 1){
        if(solid_regions1[0] > 0){
          if(solid_regions1[0] > trim_5_end){
            ...
            mic_correct_errors_5_prime_end(
                read_content,
                sequence_modified,
                read_quality,
                solid_regions1[0] - 1,
      ...
      if(solid_regions_ind >= 1){
        if(solid_regions2[solid_regions_ind - 1] < read_length - arg.kmer_length){
          if(solid_regions2[solid_regions_ind - 1] + arg.kmer_length < read_length - trim_3_end){
            ...
            mic_correct_errors_3_prime_end(
                read_content,
                sequence_modified,
                read_quality,
      ...
      if(c_path_size_first > 0){
        ...
        for(uint32_t it_candidates = 0; it_candidates < c_path_size_first; it_candidates ++){
          ...
          uint32_t c_path_size_tmp = 0;
          if(run_exploration == true){
            mic_extend_first_kmer_to_right(
                sequence_modified,
                read_quality,
                pos_path_vec_first + it_candidates * 4,
                c_path_vec_first + it_candidates * 4,
      ...
      if(c_path_size > 1){
        ...
        mic_modify_sequence(read_content, sequence_modified, read_quality, sequence_modification, trim_5_end, trim_3_end, 
