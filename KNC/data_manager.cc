...
__ONMIC__ void mic_correct(char* read_content , uint32_t data_size, uint32_t* out_size, uint32_t* read_offset, uint32_t read_offset_size, init_args& arg, uint8_t* bit_vector, uint32_t* hash_seed, stats_info& statistics, uint32_t read_iter){

  uint32_t num_threads = omp_get_num_procs();
  uint32_t read_offset_index[256];
  uint32_t read_num[256];
  while(data_size < num_threads * 4096){
    num_threads /= 2; 
  }
  if(num_threads == 0)
    num_threads = 1;
...
#pragma omp parallel for 	
  for(uint32_t i = 0; i < num_threads; i ++){
    ...
    if(i != 0){
      uint32_t n_num = 0;
      uint64_t offset_pos[8];
      uint64_t ii = read_offset_index[i] - 1;
      uint64_t j;
      uint32_t plus_num = 0;
      for(j = 0; j < 4096; j ++){
      ...
      read_offset_index[i] = read_offset_index[i] - offset_pos[n_num - 5];
    }
  }
...
#pragma omp parallel for schedule(dynamic, 10) private( min_check_length, max_allowed_ns,\
  ...
  reduction(+: num_corrected_errors_tmp, num_corrected_reads_tmp, num_trimmed_bases_tmp) 
  ...
  uint32_t total_offset = data_size; 
...
#pragma omp parallel for  
    ...
    total_offset = read_offset_index[0]; 
    for(uint32_t i = 1; i < num_threads; i ++){
      uint32_t begin = i * (data_size / num_threads);
...
void cpu_correct(char* read_content , uint32_t data_size, uint32_t *out_data_size, uint32_t* read_offset, uint32_t read_offset_size, init_args& arg, uint8_t* bit_vector, stats_info& statistics){
  ...
  if(num_threads_exe > 8)
    num_threads_exe -= 8;
  uint32_t read_offset_index[64];
  uint32_t read_num[64];
  if(data_size < num_threads_exe * 4096)
    num_threads_exe = 1; 
  uint32_t ii = 0;
...
#pragma omp parallel for num_threads(num_threads_exe)	
  for(uint32_t i = 0; i < num_threads_exe; i ++){
    ...
    if(i != 0){
      ...
      uint32_t ii = read_offset_index[i] - 1;
      uint32_t j;
      uint32_t plus_num = 0;
...
#pragma omp parallel for schedule(dynamic, 10)  private( min_check_length, max_allowed_ns,\
    max_trimmed_bases, num_corrected_errors_local, trim_5_end, trim_3_end, too_many_errors) \
  reduction(+: num_corrected_errors_tmp, num_corrected_reads_tmp, num_trimmed_bases_tmp, total_reads_num2, total_reads_num3) num_threads(num_threads_exe)
  for (uint32_t i = 0; i < total_read_num; i++) {
    ...
    if(read_offset[(i << 2) + 3] <=  max_allowed_ns && read_offset[(i << 2) + 2] >= arg.kmer_length){
      ...
      total_reads_num3 += 1;
      //for(int j = 0; j < 151; j ++)
      //    printf("%c",  read_content[read_offset[(i << 2)] + j]);
      //printf("\n");
...
void manage_data(init_args arg, C_time& c_inst_time, C_arg& c_inst_arg, uint8_t* bit_vector, int my_rank, struct stat& pfile_stat, FILE* pfile, FILE* f_corrected_read, double& mpi_total_time){
  ...
  uint32_t cpu_buffer_size = 60 * 1024 * 1024;
  uint32_t mic_num = 0; 
...
#pragma offload target(mic:(i))\
  ...
  volatile uint32_t read_iteration = device_num;
...
#pragma offload target(mic:i)\
          ...
          in(hash_seed:length(0) alloc_if(0) free_if(0))\
          in(hash_array, notrim)\
          in(compute_data_size, read_offset_size, read_iteration)\
          inout(out_data_size, statistics)
          {
            init_args arg;
            arg.notrim = notrim;
            array_2_args(hash_array, arg);
            mic_correct(mmic_buffer,compute_data_size, &out_data_size, read_offset, read_offset_size, arg, bit_vector, hash_seed, statistics, read_iteration);
          }
